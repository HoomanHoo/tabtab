...
function confirmemail() {
	...
	else if (inputform.email.value.lastIndexOf("@") == 0){
		alert(emailterror);
		inputform.email.value="";
		inputform.email.focus();
		return false;
...
function inputcheck() {
	...
	if( ! inputform.name.value ) {
		...
		return false;	
	}else if ( ! inputform.supplier_name.value) {
		alert(p_nameerror);
		inputform.supplier_name.focus();
		return false;	
	}
	else if(inputform.email.value.indexOf("@") == -1 ) {//-1이면 값이 없다 값있으면 있는 문자열의 위치를 알려줌
		alert(emailterror);
		inputform.email.value="";
		inputform.email.focus();
		return false;
